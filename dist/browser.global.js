...
(() => {
  ...
  var replayQueue = (queue) => {
    const sdkRecord = browserDataStreamApiClient;
    for (const entry of queue) {
      const [methodName, ...args] = Array.from(entry);
      const handler = sdkRecord[methodName];
      try {
  ...
  if (typeof window !== "undefined") {
    const existing = window.DataStreamApiClient;
  ...
  var browser_default = browserDataStreamApiClient;
})();
